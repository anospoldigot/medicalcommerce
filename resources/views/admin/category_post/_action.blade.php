...
<div class="modal fade" id="editModal{{ $model->id }}" tabindex="-1" aria-labelledby="editModal{{ $model->id }}Label"
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <form action="{{ route("category_post.update", $model->id) }}" method="post" class="edit">
                @csrf
                @method('PATCH')
...
<form onsubmit="return confirm('Are you sure?');" action="{{ route('category_post.destroy', $model->id) }}" method="POST"
    class="d-inline">
    @csrf
    @method('DELETE')
